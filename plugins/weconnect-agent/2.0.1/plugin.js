...
api.registerService({
  ...
  create(config) {
    ...
    // Config is passed as parameter to create() — also keep a live ref via api.config
    const cfg = () => config || api.config || {}
    let tokens      = null
    let pollTimer   = null
    ...
    async function login() {
      ...
      console.log(`[weconnect-agent] login() called — email="${email}", hasPassword=${!!password}`)
    ...
    async function poll() {
      ...
      console.log(`[weconnect-agent] poll() — email="${email}", hasPassword=${!!password}, vin="${vin}"`)
      if (!email || !password) { setStatus('error', 'Email and password not configured'); schedule(); return }
