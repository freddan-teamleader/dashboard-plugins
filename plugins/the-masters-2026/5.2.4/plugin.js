...
api.registerWidget({
  ...
  render: async function(container, config) {
    ...
    const renderTable = (players, status) => {
      ...
      // Compute tied positions client-side using player ID as unique key
      let posCounter = 1
      ...
      const rows = sorted.map(p => {
        ...
        // Use player ID as unique row identifier to avoid duplicate key warnings
    ...
    // --- Main orchestrator ---
    // players is kept in closure — persists across refreshes within the same render lifecycle
    let players = null

    // Restore cached roster (IDs + names only, no scores)
