...
api.registerWidget({
  ...
  render: async function(container, config) {
    ...
    // 2 requests per player instead of 3 — movement read from competitor page inline
    const loadScores = async (players) => {
      for (const p of players) {
        try {
          ...
          // Movement comes from the competitor page — already fetched during Phase 1 roster load,
          // but on refresh we read it from the score page text as a bonus field if present,
          // otherwise keep last known value
          const movFromScore = rxNum(scoreTxt, 'movement')
          if (movFromScore !== null) p.mov = movFromScore
          else if (p.mov == null) p.mov = 0
    ...
    const renderTable = (players, status) => {
      ...
      // Compute tied positions client-side
    ...
    // --- Main orchestrator ---
    // players held in closure — persists across all interval ticks
    let players = null

    // Restore cached roster if available — skips Phase 1 entirely on subsequent mounts
    ...
    const load = async (isRefresh = false) => {
      try {
        if (!players) {
          ...
          api.updateConfig({ cachedRoster: players.map(({ id, order, name, country }) => ({ id, order, name, country })) })
        }

        // Phase 2: scores + status only — 2 requests per player
