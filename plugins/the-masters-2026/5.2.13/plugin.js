    ...
    const loadScores = async () => {
      ...
      for (const p of players) {
        ...
        try {
          ...
          // completedRoundsDisplayValue is the reliable to-par string e.g. "-5", "E", "+3"
          ...
          if (best === 'E' || best === 'Even') {
            ...
            // If it looks like raw strokes (e.g. 67, 72) subtract PAR
    ...
    const loadLinescores = async () => {
      for (const p of players) {
        ...
        try {
          ...
          // The linescores response structure (from earlier probe):
          // {"count":2,"items":[{"value":63,"displayValue":"-5","period":1,"linescores":[...hole scores...]},{"value":71,...}]}
          // We split on each item boundary — text before "linescores" key contains round total + period
          // Split by "},{"  to separate items, then extract value+period from each chunk before "linescores"
          const chunks = t.split(/"linescores"\s*:\s*\[/)
          // chunks[0] = header, chunks[1] = first round entry, chunks[2] = second round entry, etc.
          // Each chunk starts just after the "linescores":[ of the PREVIOUS item
          // Actually better: match all top-level items by finding "value":N,"displayValue":"...","period":N
          ...
          for (const m of t.matchAll(itemRx)) {
            ...
            // Only treat as round total if strokes look like a full round (50-85)
