...
api.registerWidget({
  ...
  render: async function(container, config) {
    ...
    const toFlag = cc => {
      if (!cc) return ''
      // Handle full country names by mapping common ones
      const nameMap = {
        'usa': 'US', 'united states': 'US', 'england': 'GB', 'uk': 'GB',
        'united kingdom': 'GB', 'australia': 'AU', 'japan': 'JP',
        'south korea': 'KR', 'korea': 'KR', 'spain': 'ES', 'ireland': 'IE',
        'northern ireland': 'GB', 'scotland': 'GB', 'wales': 'GB',
        'canada': 'CA', 'south africa': 'ZA', 'norway': 'NO', 'sweden': 'SE',
        'germany': 'DE', 'france': 'FR', 'italy': 'IT', 'china': 'CN',
        'denmark': 'DK', 'belgium': 'BE', 'netherlands': 'NL', 'mexico': 'MX',
        'argentina': 'AR', 'colombia': 'CO', 'chile': 'CL', 'brazil': 'BR',
        'new zealand': 'NZ', 'thailand': 'TH', 'austria': 'AT', 'czech republic': 'CZ',
        'finland': 'FI', 'portugal': 'PT', 'switzerland': 'CH', 'india': 'IN',
        'zimbabwe': 'ZW', 'venezuela': 'VE', 'trinidad': 'TT', 'fiji': 'FJ'
      }
      const lower = cc.toLowerCase().trim()
      const iso = nameMap[lower] || (cc.length === 2 ? cc.toUpperCase() : null)
    ...
    if (!document.getElementById('masters-2026-style')) {
      ...
      s.textContent = `
        ...
        .flag-cell { font-size: 14px; line-height: 1; }
    ...
    const renderTable = arr => {
      ...
      tbody.innerHTML = arr.map(p => `
        <tr data-id="${p.id}">
          ...
          <td class="flag-cell">${toFlag(p.country)}</td>
    ...
    const loadScores = async () => {
      ...
      for (const p of players) {
        ...
        try {
          ...
          const best   = crDisp || disp || 'E'
          if (best === 'E' || best === 'Even') {
            p.totVal = 0
          } else {
            const parsed = parseFloat(best)
            p.totVal = (!isNaN(parsed) && Math.abs(parsed) <= 20) ? parsed
                     : (!isNaN(parsed)) ? Math.round(parsed) - PAR
                     : 0
          }
          ...
          const isFinished = state === 'post' || state.toLowerCase().includes('final') || state.toLowerCase().includes('complete')
    ...
    const loadLinescores = async () => {
      for (const p of players) {
        ...
        try {
          ...
          // Split on the "linescores":[ boundary so we only see round-level data
          // Each chunk before "linescores":[ contains the round total value + period
          const chunks = t.split(/"linescores"\s*:\s*\[/)
          // chunks[0] = preamble, chunks[1..N] = each round's trailing hole data
          // The round total lives in the text BEFORE each "linescores":[ marker
          // Use matchAll on chunks to find value+period at round level only
          ...
          for (const m of t.matchAll(itemRx)) {
            ...
            // Round totals are 50–85 strokes; hole scores are 1–9
