...
api.registerWidget({
  ...
  render: async function(container, config) {
    ...
    const computePositions = (arr) => {
      ...
      arr.forEach(p => { if (p.isTie) p.displayPos = `T${p.displayPos}` })
    }
    ...
    const renderTable = (arr) => {
      ...
      tbody.innerHTML = arr.map(p => `
        <tr data-id="${p.id}">
          ...
          <td class="num ${scoreClass(p.totVal)}">${p.totDisp}</td>
          ...
          <td class="num">${movHTML(p.mov)}</td>
        </tr>`).join('')
    }

    // ── Phase 1 — Roster (runs once, cached) ─────────────────────────
    let players = null
    ...
    const loadRoster = async () => {
      ...
      for (let page = 1; page <= max; page++) {
        ...
        const id  = rx(t, 'id')
        const ord = rxNum(t, 'order') ?? page
        if (!id || id === '401811941') continue
        entries.push({ id, order: ord })
        await sleep(100)
      }
    ...
    // ── Phase 2 — Scores (runs every refresh) ────────────────────────
    const loadScores = async (roster) => {
      lbl.textContent = 'Updating scores…'
      for (const p of roster) {
        try {
          // Score
          const st = await fetchText(`${COMP}/competitors/${p.id}/score`)
          p.totDisp = rx(st, 'completedRoundsDisplayValue') || rx(st, 'displayValue') || 'E'
          p.totVal  = rxNum(st, 'completedRoundsValue') ?? rxNum(st, 'value') ?? 0
          await sleep(80)

          // Status
          const ss = await fetchText(`${COMP}/competitors/${p.id}/status`)
      ...
      try {
        if (!players) {
          if (config.cachedRoster?.length) {
            players = config.cachedRoster.map(p => ({ ...p, totVal: 0, totDisp: 'E', thruDisp: '-', mov: 0, r1: '-', r2: '-', r3: '-', r4: '-', activeRound: 0 }))
          } else {
