...
api.registerWidget({
  ...
  render: async function(container, config) {
    ...
    const refreshSec = config.refreshInterval || 30;
    ...
    async function fetchFinnhub(symbol) {
      ...
      const r = await api.fetch(url);   // ← uses proxy to bypass CORS
    ...
    async function refresh() {
      ...
      if (!destroyed) {
        ...
        dot.className    = fail === symbols.length ? 'dot error' : 'dot';
        stat.textContent = fail === symbols.length ? 'Error' : 'Live';
