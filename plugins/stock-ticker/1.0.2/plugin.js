  ...
  render: async function(container, config) {
    ...
    container.innerHTML = `
      <style>
        ...
        .ticker-row {
          display: grid;
          grid-template-columns: 52px 1fr 70px 72px;
          align-items: center;
          gap: 8px;
          padding: 6px 8px;
          background: #1e293b;
        ...
        .sym  { font-size: 13px; font-weight: 700; color: #e2e8f0; }
        .name { font-size: 10px; color: #94a3b8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .spark { height: 28px; }
        ...
        <div class="ticker-list" id="ticker-list">
          ${symbols.map(s => `
            <div class="ticker-row" id="row-${s}">
              <div><div class="sym">${s}</div></div>
              <div class="name" id="name-${s}">—</div>
    ...
    const DEMO = {
      AAPL:  { c: 189.30, d: 1.24,  dp:  0.66, name: "Apple Inc.",        spark: [185,186,187,186,188,189,188,189] },
      MSFT:  { c: 415.50, d: -2.10, dp: -0.50, name: "Microsoft Corp.",   spark: [418,417,416,418,417,416,415,415] },
      GOOGL: { c: 175.80, d: 3.40,  dp:  1.97, name: "Alphabet Inc.",     spark: [171,172,173,174,173,175,176,175] },
      TSLA:  { c: 177.90, d: -5.60, dp: -3.05, name: "Tesla Inc.",        spark: [184,182,181,180,179,179,178,177] },
      NVDA:  { c: 875.40, d: 22.30, dp:  2.62, name: "NVIDIA Corp.",      spark: [850,855,860,858,865,870,872,875] },
      AMZN:  { c: 185.20, d: 0.85,  dp:  0.46, name: "Amazon.com Inc.",   spark: [183,183,184,185,184,185,185,185] },
      META:  { c: 492.60, d: 7.20,  dp:  1.48, name: "Meta Platforms",    spark: [483,485,487,490,489,491,492,492] },
      NFLX:  { c: 628.90, d: -3.40, dp: -0.54, name: "Netflix Inc.",      spark: [633,632,631,630,631,629,629,628] },
    };
    ...
    // Cache last known good data so 429s don't blank the display
    const cache = {};
    ...
    async function fetchFinnhub(symbol) {
      ...
      return { c: j.c, d: j.d, dp: j.dp, name: symbol, spark };
    }
    ...
    async function refresh() {
      ...
      for (const sym of symbols) {
        ...
        try {
          ...
          cache[sym] = data;           // store good data
          applyQuote(sym, data);
          ok++;
        } catch(e) {
          if (e.message === 'RATE_LIMIT') {
            rateLimited = true;
            // Show cached data if available, otherwise leave as-is
            if (cache[sym]) applyQuote(sym, cache[sym]);
            // Stop hammering the API — bail out of the loop
            break;
          }
          fail++;
          // On other errors, show cached data if available
          if (cache[sym]) {
        ...
        // 1s gap between requests to stay within 60 req/min
